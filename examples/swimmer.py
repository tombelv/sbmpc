...
SCENE_PATH = "examples/swimmer/swimmer.xml"
...
class Objective(BaseObjective):
    ...
    def running_cost(self, state: jnp.array, inputs: jnp.array, reference) -> jnp.float32:
        ...
        com_pos = mjx_data.xpos[self.object_site_id][0:2]
        return ((com_pos - reference[:2])**2).sum() + 0.01*inputs.transpose() @ inputs

    def final_cost(self, state, reference):
        #com_pos = state[0:1]
        mjx_data = self.robot_model.data
        com_pos = mjx_data.xpos[self.object_site_id][0:2]
        return 100*((com_pos - reference[:2]) ** 2).sum()
...
if __name__ == "__main__":
    ...
    config.sim_iterations = 20000

    # Reference for the end-effector position
    final_com_pos = jnp.array([3.0, 0.], dtype=jnp.float32)
