...
class GaussianProcessSampling(): 
    ...
    def sk_gp(self): # from sklearn 
        ...
        # gpr = GaussianProcessRegressor(kernel=kernel, random_state=0, optimizer="fmin_l_bfgs_b")
        gpr = GaussianProcessRegressor(kernel=kernel, alpha=0.0)
        gpr.fit(self.Xtr, self.ytr)

        score = gpr.score(self.Xtr, self.ytr)  # should be perfect - test with Xte and yte
        print(f"Score on training = {score}") # coefficient of determination
        
        score = gpr.score(self.Xte, self.yte)  # should be perfect - test with Xte and yte
        print(f"Score on test= {score}") # coefficient of determination

        # print(Xte.shape)
        # mean, cov = gp.predict(np.reshape(Xte[0],(1,-1)), return_cov=True) # can change to return std
        # print(f"Prediction = {pred}\n Result = {yte[13:18]}")
        # mean = jnp.reshape(mean,(mean.shape[0],1))
        # print(f"Mean = {mean.shape}, cov = {cov.shape} and X {Xte.shape}")
        # Shapes: mean = (15, 1), cov = (15, 15) and X (15, 38)
